 ...
 *     forall c:CoinTypes : 0 <= getChangeNb(c) <= MAX_COIN
 ...
 *     forall c:CoinTypes : getCashNb(d) == 0
 *     forall c:CoinTypes : getCreditNb(d) == 0
 *     forall c:CoinTypes : getChangeNb(d) == 0
 */
public interface DrinksMachineModel {
     ...
     * On peut annuler ce crédit en faisant resetCredit(), ce qui a pour effet
     ...
     * On peut prendre cette monnaie en faisant resetChange().
     */
    int getChangeAmount();
    /**
     ...
     * Le crédit est mis en caisse et la monnaie est éventuellement rendue.
     * @pre
     *     d != null
     *     getDrinkNb(d) >= 1
    ...
    /**
     * Augmente le crédit d'une pièce de type c si possible.
     * @pre
     *     c != null
     * @post
     *     old getCashNb(c) == MAX_COIN
